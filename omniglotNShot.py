	...
	def load_data_cache(self, data_pack):
		...
		for sample in range(50):  # num of episodes
			...
			for i in range(self.batchsz):  # one batch means one set
				...
				np.random.shuffle(shuffle_idx)  # [2,4,1,0,3]更改排列序列
				shuffle_idx_test = np.arange(self.n_way)  # [0,1,2,3,4]
				np.random.shuffle(shuffle_idx_test)  # [2,0,1,4,3]更改排序序列
				selected_cls = np.random.choice(data_pack.shape[0], self.n_way, False)#从大小为self.n_way生车5的均匀随机样本
				print(selected_cls)
				for j, cur_class in enumerate(selected_cls):  # for each selected cls
					...
					selected_imgs = np.random.choice(data_pack.shape[1], self.k_shot + self.k_query, False)   #2
